...
class ConditionalVarianceNorm2d(nn.Module):
  ...
  def execute(self, x, y):
    vars = jt.var(x, dim=(2, 3), keepdim=True)
    h = x / jt.sqrt(vars + 1e-5)
...
class VarianceNorm2d(nn.Module):
  def __init__(self, num_features, bias=False):
    ...
    self.num_features = num_features
    self.bias = bias
    self.alpha = nn.Parameter(jt.zeros(num_features))
...
class InstanceNorm2dPlus(nn.Module):
  def __init__(self, num_features, bias=True):
    ...
    self.instance_norm = nn.InstanceNorm2d(num_features, affine=False, track_running_stats=False)
    self.alpha = nn.Parameter(jt.zeros(num_features))
    self.gamma = nn.Parameter(jt.zeros(num_features))
    self.alpha.data.normal_(1, 0.02)
    self.gamma.data.normal_(1, 0.02)
    if bias:
      self.beta = nn.Parameter(jt.zeros(num_features))

  def execute(self, x):
    means = jt.mean(x, dim=(2, 3))
    m = jt.mean(means, dim=-1, keepdim=True)
    v = jt.var(means, dim=-1, keepdim=True)
    means = (means - m) / (jt.sqrt(v + 1e-5))
    h = self.instance_norm(x)
