...
from jittorvision_utils import make_grid, save_image
...
def train(config, workdir):
  ...
  for step in range(initial_step, num_train_steps + 1):
    batch = jt.array(next(train_iter)['image'].numpy()).float()
    ...
    if step % config.training.log_freq == 0:
      ...
      writer.add_scalar("training_loss", loss.numpy(), step)
    ...
    if step % config.training.eval_freq == 0:
      eval_batch = jt.array(next(eval_iter)['image'].numpy()).float()
    ...
    if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
      ...
      if config.training.snapshot_sampling:
        ...
        image_grid = jt.make_grid(sample, nrow, padding=2)
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_loss:
      ...
      for i, batch in enumerate(eval_iter):
        eval_batch = jt.array(batch['image'].numpy()).float()
        eval_batch = eval_batch.permute(0, 3, 1, 2)
        eval_batch = scaler(eval_batch)
