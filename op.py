...
class UpFirDn2dBackward(Function):
    ...
    def grad(self, gradgrad_input):
        ...
        # gradgrad_out = gradgrad_out.view(self.in_size[0], self.out_size[0], self.out_size[1], self.in_size[3])
...
class UpFirDn2d(Function):
    def execute(self, input, kernel, up, down, pad):
        ...
        self.kernel = kernel
        self.flipped_kernel = jt.misc.flip(kernel, [0,1])
    ...
    def grad(self, grad_output):
        kernel = self.kernel
        grad_kernel = self.flipped_kernel
...
def upfirdn2d(input, kernel, up=1, down=1, pad=(0, 0)):
    '''Interface Function'''
    if False: # input.device.type == "cpu":
